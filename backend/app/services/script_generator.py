    ...
    def generate_python_script(filename: str, steps: List[TransformationStep]) -> str:
        ...
        for idx, step in enumerate(steps, 1):
            ...
            if op == "trim_text":
                ...
                code_lines.append(f'        df[{col_lit}] = df[{col_lit}].astype(str).apply(lambda x: re.sub(r"\\s+", " ", x.strip()) if pd.notna(x) and x.lower() != "nan" else x)')
                ...
                code_lines.append(f'    if {col_lit} in df.columns:')
                code_lines.append(f'        df[{col_lit}] = pd.to_datetime(df[{col_lit}], dayfirst=True, format="mixed", errors="coerce").dt.strftime({fmt_lit})')

            elif op == "convert_numeric":
                code_lines.append(f'    if {col_lit} in df.columns:')
                code_lines.append(f'        cleaned = df[{col_lit}].astype(str).str.replace("€", "").str.replace("$", "").str.replace("%", "").str.strip()')
                ...
                code_lines.append(f'    if {col_lit} in df.columns:')
                code_lines.append(f'        df[{col_lit}] = pd.to_numeric(df[{col_lit}], errors="coerce").round({decimals})')
                ...
                if strat == "mean":
                    code_lines.append(f'        df[{col_lit}] = df[{col_lit}].fillna(pd.to_numeric(df[{col_lit}], errors="coerce").mean())')
                elif strat == "median":
                    code_lines.append(f'        df[{col_lit}] = df[{col_lit}].fillna(pd.to_numeric(df[{col_lit}], errors="coerce").median())')
                else:
