...
async def functional_exception_handler(request: Request, exc: FunctionalException):
    ...
    return JSONResponse(
        ...
        content={"error": True, "code": exc.code, "message": exc.message, "details": exc.details},
    )
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    logger.error(f"[error_id={error_id}] Unhandled server error on {request.url}: {str(exc)}\n{error_traceback}")
