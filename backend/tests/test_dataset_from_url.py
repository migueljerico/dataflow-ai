...
def test_import_dataset_from_url_success():
    ...
    with patch("app.core.security_url.validate_and_resolve_url") as mock_val, \
        ...
        response = client.post(
            ...
            json={"url": "https://public-open-data.org/sales.csv"}
        )
...
def test_import_dataset_from_url_blocked_ipv6_loopback():
    ...
    response = client.post(
        "/api/v1/datasets/from-url",
        json={"url": "http://[::1]:8000/internal-data.csv"}
    )
    assert response.status_code == 400
    data = response.json()
    assert data["error"] is True
...
def test_import_dataset_from_url_blocked_ipv4_mapped_ipv6():
    ...
    response = client.post(
        "/api/v1/datasets/from-url",
        json={"url": "http://[::ffff:127.0.0.1]:8000/internal-data.csv"}
    )
    assert response.status_code == 400
    data = response.json()
    assert data["error"] is True
...
def test_import_dataset_from_url_oversized_content_length():
    ...
    with patch("app.core.security_url.validate_and_resolve_url") as mock_val, \
        ...
        mock_client_cls.return_value = _setup_stream_mock(
            ...
            byte_chunks=[b"small chunk"]
        )

        response = client.post(
            "/api/v1/datasets/from-url",
            json={"url": "https://public-open-data.org/huge_file.csv"}
        )
...
def test_import_dataset_from_url_empty_file():
    ...
    with patch("app.core.security_url.validate_and_resolve_url") as mock_val, \
        ...
        response = client.post(
            "/api/v1/datasets/from-url",
            json={"url": "https://public-open-data.org/empty.csv"}
        )
...
def test_import_dataset_from_url_remote_404():
    ...
    with patch("app.core.security_url.validate_and_resolve_url") as mock_val, \
        ...
        response = client.post(
            "/api/v1/datasets/from-url",
            json={"url": "https://public-open-data.org/not_found.csv"}
        )
