...
export const BatchPlanReview: React.FC<Props> = ({
  ...
  const getRiskBadge = (risk: string) => {
    if (risk === 'high') return <span className="badge badge-rose">{t.batchPlan?.riskHigh ?? 'Alto'}</span>;
    if (risk === 'medium') return <span className="badge badge-amber">{t.batchPlan?.riskMedium ?? 'Medio'}</span>;
    return <span className="badge badge-emerald">{t.batchPlan?.riskLow ?? 'Bajo'}</span>;
  };
    ...
    <div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          <div>
            <h2 style={{ fontSize: '1.3rem', fontWeight: 800, marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Sparkles size={20} className="text-primary" /> {(t.batchPlan?.planTitle ?? 'Plan de Transformación Integral — {n} Tablas').replace('{n}', String(batchPlans.length))}
            </h2>
          ...
          >
            ...
            <span>{executing ? (t.batchPlan?.cleaningAllTables ?? 'Limpiando todas las tablas...') : (t.batchPlan?.approveCleanAll ?? 'Aprobar y Limpiar Todas ({n} pasos)').replace('{n}', String(totalSteps))}</span>
          </button>
        </div>
      </div>
            ...
            <div key={datasetId} className="card" style={{ padding: '0', overflow: 'hidden' }}>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <span className="badge badge-blue">{(t.batchPlan?.stepsBadge ?? '{n} pasos').replace('{n}', String(plan.steps.length))}</span>
                </div>
                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                  {plan.summary || (t.batchPlan?.autoGeneratedPlan ?? 'Plan generado automáticamente')}
                </div>
              </div>
                ...
                <div style={{ padding: '16px 20px' }}>
                  {plan.steps.length === 0 ? (
                    <div style={{ color: 'var(--accent-emerald)', fontSize: '0.875rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <CheckCircle2 size={16} /> {t.batchPlan?.noIssuesMsg ?? 'Esta tabla no contiene anomalías detectadas; ya está limpia.'}
                    </div>
                  ) : (
                        ...
                        >
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                              ...
                              {step.column && (
                                <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                                  {t.batchPlan?.inColumn ?? 'en '}<code>{step.column}</code>
                                </span>
                              )}
                          ...
                          {step.affected_rows_estimate !== undefined && step.affected_rows_estimate > 0 && (
                            <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textAlign: 'right' }}>
                              {(t.batchPlan?.affectsRows ?? 'Afecta ~{n} filas').replace('{n}', String(step.affected_rows_estimate))}
                            </div>
                          )}
                        </div>
                      ))}
      ...
      <div className="card" style={{ textAlign: 'center', padding: '20px' }}>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', marginBottom: '16px' }}>
          {t.batchPlan?.governanceLabel ?? 'Principio Fundamental de Gobierno: '}<em>{t.batchPlan?.governanceStatement ?? 'La IA propone. El usuario decide con el botón. Python ejecuta de forma determinista.'}</em>
        </p>
        <button
          type="button"
          className="btn btn-success"
        ...
        >
          ...
          <span>{executing ? (t.batchPlan?.cleaningBatch ?? 'Limpiando todas las tablas en lote...') : (t.batchPlan?.approveExecuteClean ?? 'Aprobar y Ejecutar Limpieza ({n} pasos)').replace('{n}', String(totalSteps))}</span>
        </button>
      </div>
    </div>
  );
